...
sub decompress
    ...
    #my $precision_multiplier = Math.pow(10, -$precision);
    my @decoded;
	...
	push( @decoded, $latlong[$latlong_ind] );
	}
    return @decoded;
    }
...
sub compress
	...
	my $diff = ($latlong - $last_latlong[$latlong_ind]);
	print "latlong=$latlong diff=$diff\n";
	push( @encoded, &encodeNumber($diff) );
